...
# FIXME psyco
# FIXME more efficient use of existing tools: Remesh, blender utils...
...
def get_global_mesh(ob):
    ...
    me = ob.to_mesh(bpy.context.scene, True, "PREVIEW") # apply modifiers
    me.transform(ob.matrix_world) # transform mesh in global coordinates
    return me

def get_bmesh_faces(me):
    """get bmesh faces"""
    try:
...
def is_manifold(me):
    ...
    edgeusers = dict()
    faces = get_bmesh_faces(me) # mod for Blender 2.63 bmesh
...
def _triangulate(me):
    """triangulation, return list of fa3 faces"""
    # fa3s is a list containing Fa3 face objects
    # me.faces -> fa3s, face -> fa3
    fa3s = list()
    for face in get_bmesh_faces(me): # mod for Blender 2.63 bmesh
...
"""
    ...
    # voxels are exported as a set
    t4 = time()

    voxels = set([(voxel[0], voxel[1], voxel[2]) for voxel in voxels]) # no more linked to me
    bpy.data.meshes.remove(me)
    print("   voxelize:", "tri:", int(t1-t0), "verts:", int(t2-t1), "hollow:", int(t3-t2), "solid:", int(t4-t3),"unlink:", int(time()-t4), "", "tot:", int(time()-t0))
    return voxels
...
def get_boxels(ob, voxel_size, solid):
    ...
    boxes.sort()
    print("   boxelize:", int(time()-t0), "seconds")
...
def get_mesh_cells(ob):
    ...
    # prevent division by zero, when solid is flat (discovered while in Sydney)
    ijk = (int(round((bbmaxx - bbminx) / cell_size[0])) or 1,
           _n_for_poisson(int(round((bbmaxy - bbminy) / cell_size[1]))) or 1,
           _n_for_poisson(int(round((bbmaxz - bbminz) / cell_size[2]))) or 1
          )
    quantity = ijk[0] * ijk[1] * ijk[2]
    cell_size = ((bbmaxx - bbminx) / ijk[0] or .001,
                 (bbmaxy - bbminy) / ijk[1] or .001,
                 (bbmaxz - bbminz) / ijk[2] or .001
                )
