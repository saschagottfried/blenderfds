...
from Geometry  import TriangleNormal
...
def get_global_mesh(ob):
    ...
    me = ob.create_mesh(True, "PREVIEW") # apply modifiers to mesh
    me.transform(ob.matrix) # transform mesh in global coordinates
    return me
    
def is_mesh(ob):
    """Check if object ob is of MESH type"""
...
def _is_inside_z_projection(point, fa3s):
    ...
    boundary_edges = set()
    for fa3 in fa3s:
        for edge in ((fa3.verts[0], fa3.verts[1]),
                     (fa3.verts[1], fa3.verts[2]),
                     (fa3.verts[2], fa3.verts[0])):
...
def _triangulate(me):
    ...
    for face in me.faces:
        fa3s.append(_Fa3(verts=[me.verts[face.verts[0]],
                                me.verts[face.verts[1]],
                                me.verts[face.verts[2]]],
                                normal=face.normal))
        if len(face.verts) != 3: # If it's a quad, add another face
            fa3s.append(_Fa3(verts=[me.verts[face.verts[0]],
                                    me.verts[face.verts[2]],
                                    me.verts[face.verts[3]]],
                                    normal=face.normal))
    return fa3s
...
"""
    ...
    if voxel_size[0] <= 0. or voxel_size[1] <= 0. or voxel_size[2] <= 0.:
        raise ValueError("Cubic voxel size must be a positive float number!")
    # Get ob mesh in global coordinates and triangulate it
    me = get_global_mesh(ob)
    fa3s = _triangulate(me)
    # Get bin vertices: voxels_verts
    # it uses vertices from Blender mesh me
    voxels_verts = dict() # {vert obj: (3,7,3), vert obj: (1,2,3), ...}
    for vert in me.verts:
        coords = vert.co
        voxels_verts[vert] = (int(round(coords[0]/voxel_size[0])),
        ...
        for fidx in range(min_fidx, max_fidx + 1):
            ...
            for fidy in range(min_fidy, max_fidy + 1):
                ...
                for fidz in range(min_fidz, max_fidz + 1):
                    ...
                    verts_wrk = [vert.co - cc for vert in verts]
                    if not (-r0 <= a0 * verts_wrk[0] <= r0): continue
                       # voxel not intersecting fa3 hyperplane
                    # Check overlap of voxel with fa3 (separating axis theorem)                     
                    fa3s_wrk = [verts_wrk[1] - verts_wrk[0],
                                verts_wrk[2] - verts_wrk[1],
                                verts_wrk[0] - verts_wrk[2]]
                    overlap = True
                    for fa3_wrk in fa3s_wrk:
                        ...
                        for e in es:
                            ...
                            ds = [a * vert_wrk for vert_wrk in verts_wrk]
                            ds.sort()
...
def _test_voxelize(context, voxel_size):
    """Test _voxelize function"""
    ...
    try:
        ...
        pass
    # Then start
    for ob in [obs for obs in context.scene.objects if obs.is_visible()]: #visible?
    #for ob in bpy.context.selected_objects:
        # Checks
        if not is_mesh(ob): continue
        if not is_manifold(ob.data): continue
        # Voxelization
        print("Voxelizing:", ob.name)
        solid = True
        voxels = _voxelize(context, ob, solid)
        # Create a new mesh with result and add vertices in flat coords
        me_new = bpy.data.meshes.new(ob.name+"_voxelized")
        me_new.add_geometry(len(voxels), 0, 0)
        flat_coords = list()
        for voxel in voxels:
            flat_coords.append(voxel[0] * voxel_size[0])
            flat_coords.append(voxel[1] * voxel_size[1])
            flat_coords.append(voxel[2] * voxel_size[2])
        me_new.verts.foreach_set("co", flat_coords)
        # Create a new object, link the new mesh, link the object to the scene
        ob_new = bpy.data.objects.new(ob.name+"_voxelized", me_new)
...
def _test_get_boxels(context, voxel_size):
    """Test get_boxels function"""
    # Import Psyco if available
    try:
        import psyco
        psyco.full()
    ...
    for ob in [obs for obs in context.scene.objects if obs.is_visible()]: #visible?
    #for ob in bpy.context.selected_objects:
        timing("Treating {}".format(ob.name,))
        if not is_mesh(ob): continue
        if not is_manifold(ob.data): continue
        solid = True
        timing("Start")
        boxes = get_boxels(context, ob, solid)
        timing("boxelize")
        print("boxes", boxes)
    print("Done")
...
def get_bb(ob):
    ...
    bbminx, bbminy, bbminz = me.verts[0].co
    bbmaxx, bbmaxy, bbmaxz = me.verts[0].co
    for vert in me.verts:
        x, y, z = vert.co
...
def get_faces(ob):
    ...
    me = get_global_mesh(ob)
    for face in me.faces:
        verts = [me.verts[vert] for vert in face.verts]
        # Calc the bounding box in global coordinates
        bbminx, bbminy, bbminz = verts[0].co
        bbmaxx, bbmaxy, bbmaxz = verts[0].co
        for vert in verts[1:]:
            x, y, z = vert.co
            bbminx, bbminy, bbminz = min(bbminx, x), min(bbminy, y), min(bbminz, z)
            bbmaxx, bbmaxy, bbmaxz = max(bbmaxx, x), max(bbmaxy, y), max(bbmaxz, z)
...
def get_edges(ob):
    ...
    for edge in me.edges:
        pt0x, pt0y, pt0z = me.verts[edge.verts[0]].co
        pt1x, pt1y, pt1z = me.verts[edge.verts[1]].co
...
def get_mesh_cells(ob):
    ...
    bbminx, bbmaxx, bbminy, bbmaxy, bbminz, bbmaxz = get_bb(ob)[0]
...
if __name__ == "__main__":
    ...
    voxel_size = (.2,.2,.2)
    time0 = time.time()
    _test_get_boxels(bpy.context, voxel_size)
    time1 = time.time()
